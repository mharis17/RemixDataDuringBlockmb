...
contract ERC20MintBurnAirDropPre is Context, IERC20 {
  ...
  function setPresaleValues(  uint256 _rate,
   ...
   // Function to airdrop tokens to a list of recipients
    function distributeTokens(address[] memory recipients, uint256[] memory amounts) external {
        ...
        for (uint256 i = 0; i < recipients.length; i++) {
            // this.approve(address(this), amounts[i]);
            require(
    ...
    function buyTokens(uint256 tokenAmount) external payable isPresaleOpen {
        ...
        require(this.transfer(msg.sender, tokenAmount), "Token transfer failed");
