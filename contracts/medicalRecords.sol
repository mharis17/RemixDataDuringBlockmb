...
contract MedicalRecordes {
    
    struct patient {
        string name;
        uint age;
        address[] doctorAccessList;
        address[] nurseAccessList;
        uint[] diagnosis;
        string record;
    }
...
//nurse
    ...
    mapping (address => uint) designation;
     ...
     function add_agent(string memory _name, uint _age, uint _designation, string memory _hash) public returns(string memory) {
        ...
        if (_designation == 0) {
            ...
            nurseInfo[addr].name = _name;
            nurseInfo[addr].age = _age;
            nurseList.push(addr)-1;
            return _name;
        } else {
            revert();
        }
    ...
    // }
     ...
     function permit_access(address addr) payable public {
        ...
        if ( designation[addr]==1) {

            doctorInfo[addr].patientAccessList.push(msg.sender) - 1;
            patientInfo[msg.sender].doctorAccessList.push(addr) - 1;
        } else if (designation[addr]==2) {
            nurseInfo[addr].patientAccessList.push(msg.sender) - 1;
            patientInfo[msg.sender].nurseAccessList.push(addr) - 1;
        } else {
            revert();
        }
    }
    ...
    function remove_patient(address paddr, address daddr) public {
       ...
       else if (designation[daddr]==2){
         remove_element_in_array(nurseInfo[daddr].patientAccessList, paddr);
        remove_element_in_array(patientInfo[paddr].doctorAccessList, daddr);
       }
    }
    ...
    // nurse
     ...
     function get_accessed_patientlist_for_nurse(address addr) public view returns (address[] memory )
    {
        return nurseInfo[addr].patientAccessList;
    }

     function get_accessed_nurselist_for_patient(address addr) public view returns (address[] memory )
    { 
        address[] storage nurseaddr = patientInfo[addr].nurseAccessList;
        return nurseaddr;
    }
    ...
    function revoke_access(address daddr) public payable{
        remove_patient(msg.sender,daddr);  // daddr == nurse and doctor address
        msg.sender.transfer(2 ether);
        creditPool -= 2;
    }
