...
contract AirdropContract is Context  {
    ...
    // IERC20 public token; // The ERC-20 token being airdropped
    // uint256 public approvedAmount; // Amount of tokens approved by the owner

    uint256  public SimpleFee;
    // address public  feeCollector;

struct AirdropDetails{
      uint256 approvedToken; // Amount of tokens approved by the owner
      bool Anyone;// anyone claim on not
      uint256 FixedAmount;// if anyone is selected than this amount of token can be airdrop 
   ...
   mapping (address=> mapping (address=>uint256) ) public claimedDataEachUser ;
    ...
    // modifier withinAirdropWindow() {
    //     require(block.timestamp >= startTime && block.timestamp <= endTime, "Airdrop is not active");
    //     _;
    // }


// function set Value ()public 
    // Owner approves the contract to transfer tokens
    function approveContract(uint256 _amount,address _tokenAddress) external  {
       IERC20 token = IERC20(_tokenAddress); // Initialize with the ERC-20 token contract address

        require(_amount > 0, "Amount should be greater than 0");
        require(token.balanceOf(owner) >= _amount, "Not enough tokens in the owner's balance");

        AirDropDetailsMap[_tokenAddress].approvedToken = _amount;
...
function AddcontractDetails (address _tokenAddress,uint256 _approvedToken,bool _Anyone,uint256 _FixedAmount,uint256 _SimpleFee) public  {

  AirDropDetailsMap[_tokenAddress].approvedToken=_approvedToken;
  ...
  AirDropDetailsMap[_tokenAddress].AlreadyClaimedAmount=0;
   ...
   AirDropDetailsMap[_tokenAddress].SimpleFee=_SimpleFee;
...
if (AirDropDetailsMap[_tokenAddress].Anyone==true){
 ...
 require(token.balanceOf(address(this)) >= AirDropDetailsMap[_tokenAddress].FixedAmount, "Not enough tokens in the contract");
       AirDropDetailsMap[_tokenAddress].AlreadyClaimedAmount+= AirDropDetailsMap[_tokenAddress].FixedAmount;
        ...
        token.transfer(msg.sender, AirDropDetailsMap[_tokenAddress].FixedAmount);
      
}
else{

        AirdropUser storage user = specificUsers[_tokenAddress][msg.sender];
        require(token.balanceOf(address(this)) >= user.amount, "Not enough tokens in the contract");
    ...
    // Check the balance of tokens in the contract
    ...
    // Check the remaining approved tokens
    function getRemainingApprovedTokens(address _tokenAddress) external view returns (uint256) {
     IERC20   token = IERC20(_tokenAddress); // Initialize with the ERC-20 token contract address

        return  AirDropDetailsMap[_tokenAddress].AlreadyClaimedAmount - token.allowance(owner, address(this));
        // return  specificUsers[_tokenAddress][].AlreadyClaimedAmount - token.allowance(owner, address(this));
...
// contract ERC20Simple is Context, IERC20 {
...
//   constructor(string memory Name,string memory Symbol ,uint256 _initialSupply,uint8 decimal ,uint256 _SimpleFee,address  payable _feeCollector  ) payable  {
//     require (msg.value>= _SimpleFee,"not enough fee");
//     _name = Name;
//     _symbol = Symbol ;
//     _decimals = decimal;
//     Contractowner=msg.sender;
//     _totalSupply = _initialSupply *10**_decimals;//in hexadecimal
//     SimpleFee=_SimpleFee;
